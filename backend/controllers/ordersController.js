...
async function createCheckout(req, res) {
    ...
    try {
        ...
        res.status(500).json({
            message: `Fehler beim Erstellen des Checkouts 33: ${error.message}`
        });
    }
};
...
async function getCheckoutById(req, res) {
    ...
    try {
        ...
        res.status(500).json({
            message: `Fehler beim Abrufen des Checkouts: ${error.message}`
        });
    }
};
async function getAllCheckouts(req, res) {
    try {
        ...
        res.status(500).json({
            message: `Fehler beim abrufen der Checkout ${error.message}`
        });
    }
}
async function getCheckoutsByUserId(req, res) {
    const {
        id
    } = req.params;
    ...
    try {
        let userId = id;
        const checkouts = await Checkout.getByUserId(userId);
        const checkoutsWithItems = await Promise.all(checkouts.map(async (checkout) => {
            const items = await CheckoutItem.getByCheckoutId(checkout.checkoutId);
            return {
                ...checkout,
        ...
        res.status(500).json({
            message: `Fehler beim abrufen der Checkouts für  nutzer: ${error.message}`
        });
    }
}
